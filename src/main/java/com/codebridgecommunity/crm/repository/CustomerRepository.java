...
public interface CustomerRepository extends ReactiveCrudRepository<Customer, Long>, CustomerRepositoryInternal {
    ...
    @Query("SELECT * FROM customer entity WHERE entity.company_id = :id")
    Flux<Customer> findByCompany(Long id);

    @Query("SELECT * FROM customer entity WHERE entity.company_id IS NULL")
    Flux<Customer> findAllWhereCompanyIsNull();
