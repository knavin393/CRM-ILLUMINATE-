...
public interface ProductOrderRepository extends ReactiveCrudRepository<ProductOrder, Long>, ProductOrderRepositoryInternal {
    ...
    @Query("SELECT * FROM product_order entity WHERE entity.product_id = :id")
    Flux<ProductOrder> findByProduct(Long id);

    @Query("SELECT * FROM product_order entity WHERE entity.product_id IS NULL")
    Flux<ProductOrder> findAllWhereProductIsNull();
